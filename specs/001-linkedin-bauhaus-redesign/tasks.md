...
- [x] Confirmar decisiones D1–D5 con Alberto — resuelto 2026-07-29, ver
      `requirements.md` §7.
- [x] Crear `content/profile.ts` implementando el contrato de `design.md` §5, con los
...
- [x] Definir el ritmo vertical estándar entre secciones (`py-24 sm:py-32`) como
      convención documentada (comentario en el propio archivo o en este spec).
- [x] Confirmar que los 4 colores de acento (`red`, `blue`, `navy`, `yellow`) están
...
- [x] `components/section-heading.tsx` — H2 + número de índice + línea divisoria.
- [x] `components/section-index-nav.tsx` — índice fijo lateral (desktop) + overlay mobile,
      con `aria-current` en la sección activa vía scroll-spy.
- [x] `components/timeline-item.tsx` — nodo + tarjeta, soporta agrupación de roles
      concurrentes (`concurrentGroup`).
- [x] `components/skill-grid.tsx` — grid de categorías/chips.
- [x] `components/highlight-tile.tsx` — tile de logro/estadística.
- [x] `components/contact-links.tsx` — extraer los 3 CTAs (email/GitHub/LinkedIn) del
      hero actual (`app/page.tsx`) a un componente reutilizable, sin cambiar su
      comportamiento actual.
- [x] `components/theme-toggle.tsx` — botón claro/oscuro según `design.md` §10.1
...
- [x] **Hero** (`app/page.tsx`): reemplazar badge "En desarrollo" por el titular real
      ...
      base.
- [x] **Gap técnico del hero:** ajustar `components/bauhaus-background.tsx` para que la
...
- [x] **Sobre mí** (`app/sections/about.tsx` o equivalente): copy de dos párrafos según
      `design.md` §4.2, usando `content/profile.ts.summary`.
- [x] **Trayectoria** (`app/sections/experience.tsx`): render de las 8 entradas de
      `profile.experience` vía `TimelineItem`, fechas calculadas con `formatDuration`,
      roles 2011–presente agrupados visualmente.
- [x] **Stack** (`app/sections/stack.tsx`): `SkillGrid` con las 3 categorías de
      `profile.skills`.
- [x] **Reconocimientos** (`app/sections/highlights.tsx`): 3–4 `HighlightTile` con los
      datos de `design.md` §4.5.
- [x] **Formación & Idiomas** (`app/sections/education.tsx`): bloque de dos columnas con
      `profile.education`, `profile.languages`, `profile.certifications`.
- [x] **Contacto/Footer** (`app/sections/contact.tsx`): CTA final + `ContactLinks` +
      copyright dinámico (reutilizar el ya existente).
- [x] Ensamblar todas las secciones en `app/page.tsx` en el orden de `design.md` §1,
      cada una envuelta con el hook de scroll-reveal (`useInViewAnimation`).

## M4 — Movimiento

- [x] Aplicar scroll-reveal (`design.md` §3.2) a la entrada de cada sección.
- [x] Aplicar stagger a las listas de Timeline/Stack/Highlights (máx. 80ms entre ítems).
- [x] Estados hover en `TimelineItem` y `HighlightTile` (color de acento, sin layout
      shift).
- [x] Scroll-spy funcional en `SectionIndexNav` (sección activa resaltada).
- [x] Extender el patrón de pausa por `visibilitychange` del canvas del hero con
      `IntersectionObserver` sobre su propio contenedor, para pausar el render cuando el
      hero sale del viewport en scroll profundo.
- [x] Verificar que `prefers-reduced-motion: reduce` desactiva **todos** los efectos
      nuevos (no solo el canvas), mostrando el contenido ya visible.
- [x] Hacer que `components/bauhaus-background.tsx` reaccione al cambio de tema
      (`design.md` §10.2): observar la clase `dark` en `<html>` y swapear el color base
      `cream`↔`ink` sin reiniciar el loop de animación.
- [x] Ajustar el `mix-blend-mode` del grano/textura en `app/page.tsx` según el tema
      activo (`multiply` en claro, `screen` en oscuro — `design.md` §10.2).

## M5 — Accesibilidad

- [x] Skip link "Saltar al contenido" antes del índice de navegación.
- [x] Jerarquía de encabezados válida (un solo H1, H2 por sección, sin saltos de nivel).
- [x] `aria-label`/`aria-labelledby` en cada `<section>`.
- [x] `aria-current` en el link activo del índice de navegación.
- [x] Verificar contraste AA de todo texto sobre fondo animado (usar herramienta de
...
- [x] Navegación completa por teclado (Tab a través de índice, CTAs, links del footer;
      sin trampas de foco).
- [ ] Auditoría con Lighthouse Accessibility (objetivo ≥ 95, `requirements.md` §6).

## M6 — Rendimiento y SEO

- [x] Confirmar que solo hay una escena WebGL activa (el hero); ninguna sección nueva
      instancia Three.js adicional.
- [x] `app/layout.tsx`: metadata OG completa + imagen OG generada con `next/og`
      (paleta/tipografía Bauhaus, usando `profile.name` + `profile.headline`).
- [x] JSON-LD `Person` (schema.org) en el `<head>`, poblado desde `content/profile.ts`.
- [x] `app/sitemap.ts` y `app/robots.ts` (convención Next.js App Router).
- [ ] Lighthouse: Performance ≥ 90 (mobile), LCP < 2.5s, CLS < 0.1, SEO 100.

## M7 — QA final y despliegue

- [x] Revisión de contenido: cada dato visible en el sitio se contrasta contra
      `requirements.md` §2 (checklist manual, cumple HU-9).
- [x] Verificación visual en 375px, 768px, 1440px (mínimo).
- [x] `npm run build` y `npm run lint` sin errores.
- [x] Captura de pantalla en **ambos temas (claro y oscuro)** para validar composición
